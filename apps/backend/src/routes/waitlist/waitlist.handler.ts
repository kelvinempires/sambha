...
export const joinWaitlistHandler: AppRouteHandler<JoinWaitlistRoute> = async (
  c
) => {
  try {
    ...
    return c.json(
      {
        ...
        data: {
          ...
          message:
            "Successfully joined waitlist. Please check your email for verification.",
        },
      },
      HttpStatusCodes.OK
    );
...
export const verifyEmailHandler: AppRouteHandler<VerifyEmailRoute> = async (
  c
) => {
  try {
    ...
    try {
      const totalReferrals = await WaitlistService.getUserStats({
        email: user.email,
      });
      const totalStats = await WaitlistService.getTotalStats();
    ...
    if (user.referrer) {
      try {
        const referrerStats = await WaitlistService.getUserStats({
          email: user.referrer,
        });
    ...
    return c.json(
      {
        ...
        data: {
          ...
          message:
            "Email verified successfully! You can now start referring friends.",
        },
      },
      HttpStatusCodes.OK
    );
    ...
    if (error instanceof Error) {
      ...
      if (
        error.message === WAITLIST_ERRORS.EMAIL_ALREADY_VERIFIED ||
...
export const getUserStatsHandler: AppRouteHandler<GetUserStatsRoute> = async (
  c
) => {
  try {
...
export const getLeaderboardHandler: AppRouteHandler<
  GetLeaderboardRoute
> = async (c) => {
...
export const getTotalStatsHandler: AppRouteHandler<GetTotalStatsRoute> = async (
  c
) => {
